...
a, b, N, alpha, h = 0, 1, 1024, 0.5, np.sqrt(0.5)
epochs = 20000

order = 2
Diffeqf = "first"
x = np.linspace(a, b, N, endpoint=False)[1:]  # trainning data: for x values without considering the end points
...
fig1 = plt.figure(1)
plt.plot(x_predict, y_predict, color="k", linewidth=4, label="Neural network solution")
...
with open('Output.csv', 'w') as f:
    ...
    writer.writeheader()
    for x in x_predict:
        writer.writerow({'x': x, 'y': str(*y_predict[n])})
        n += 1
